function Sync-HomeScripts {
  ...
  $files = try {
    $listUrl = "${baseRawUrl}/.brbaro.list?cb=$buster"
    $content = Invoke-RestMethod -Uri $listUrl -UseBasicParsing -ErrorAction Stop
  ...
  foreach ($fileName in $files) {
    ...
    $url = "${baseRawUrl}/${fileName}?cb=$buster"

    try {
      $finalContent = Invoke-RestMethod -Uri $url -UseBasicParsing -ErrorAction Stop
      ...
      if ($shouldWrite) {
        ...
        Write-Host "  [+] Synced: $fileName" -ForegroundColor Gray
      } else {
...
if ($args -contains "-x") {
  ...
  $bootUrl = "https://raw.githubusercontent.com/brbaro/home/refs/heads/main/Scripts/up.ps1?cb=$(Get-Random)"
  Invoke-RestMethod -Uri $bootUrl -UseBasicParsing | Invoke-Expression
} else {
  Sync-HomeScripts
}
